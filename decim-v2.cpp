...
void ECRYPT_Decim::ECRYPT_keysetup(void* ctxa, const u8* key, u32 keysize, u32 ivsize)
{
...
void ECRYPT_Decim::ECRYPT_ivsetup(void* ctxa, const u8* iv)
{
  DECIM_ctx* ctx = (DECIM_ctx*)ctxa;
  int i, j;
 ...
 */
void ECRYPT_Decim::DECIM_process_bytes(int action, void* ctxa, const u8* input, u8* output, u32 msglen)
{
  DECIM_ctx* ctx = (DECIM_ctx*)ctxa;
  u8 stream_byte;
...
void ECRYPT_Decim::ECRYPT_encrypt_bytes(void* ctx, const u8 * plaintext, u8 * ciphertext, u32 msglen) {
    DECIM_process_bytes(0, ctx, plaintext, ciphertext, msglen);
}
void ECRYPT_Decim::ECRYPT_decrypt_bytes(void* ctx, const u8 * ciphertext, u8 * plaintext, u32 msglen) {
    DECIM_process_bytes(1, ctx, ciphertext, plaintext, msglen);
}
